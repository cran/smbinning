...
#' utilizes \href{http://cran.r-project.org/web/packages/partykit}{Recursive Partitioning} to categorize 
#' the numeric characteristic.\cr
#' The especific algorithm is 
#' \href{http://cran.r-project.org/web/packages/partykit/vignettes/ctree.pdf}{Conditional Inference Trees} 
...
smbinning = function(df,y,x,p=0.05){
  ...
  if (!is.numeric(df[,i])){ 
    ...
    ivt[,9]=round(ivt[,4]/ivt[,2],4) # BadRate
    ivt[,10]=round(ivt[,3]/ivt[,4],4) # Odds
    ivt[,11]=round(log(ivt[,3]/ivt[,4]),4) # LnOdds
    G=ivt[i+2,3]
    B=ivt[i+2,4]
    LnGB=log(G/B) # IV Part 1
    ivt[,12]=round(log(ivt[,3]/ivt[,4])-LnGB,4) # WoE
    ivt[,13]=round(ivt[,12]*(ivt[,3]/G-ivt[,4]/B),4) # Mg IV
    ivt[i+2,13]=round(sum(ivt[,12]*(ivt[,3]/G-ivt[,4]/B),na.rm=T),4)
    iv=ivt[i+2,13]
    # End Inf. Value Table ###################################################### 
    }
  bands=append(mincutpoint,cutvct)
  bands=append(bands,cutpoint)
  list(ivtable=ivt,iv=iv,ctree=ctree,bands=bands,x=x,col_id=j)
  }
...
#' @param option Distribution ("dist"), Bad Rate ("badrate"), and Weight of Evidence ("WoE").
...
smbinning.plot=function(ivout,option="dist",sub=""){
  ...
  if (option=="dist"){
    ...
    ch_badrate=barplot(ivout$ivtable[1:x_upper,9],
                       names.arg=ivout$ivtable[1:x_upper,1],
                       axes=F, 
                       main="Bad Rate (%)",
                       ylim=c(0,y_upper),
                       col=gray.colors(length(unique(ivout$ivtable[1:x_upper,1]))))
    text(x=ch_badrate,y=ivout$ivtable[1:x_upper,9], label=round(ivout$ivtable[1:x_upper,9]*100,1), pos=3,cex=1)
    abline(h=0)
    mtext(sub,3)
  } else if (option=="WoE") {
    # WoE
    x_upper=nrow(ivout$ivtable)-r
    y_upper=max(ivout$ivtable[1:x_upper,12],na.rm=T)*1.25
    y_lower=min(ivout$ivtable[1:x_upper,12],na.rm=T)*1.25
    ch_woe=barplot(ivout$ivtable[1:x_upper,12],
                   names.arg=ivout$ivtable[1:x_upper,1],
                   axes=F, 
                   main="Weight of Evidence", 
                   ylim=c(y_lower,y_upper),
                   col=gray.colors(length(unique(ivout$ivtable[1:x_upper,1]))))
    text(x=ch_woe,y=ivout$ivtable[1:x_upper,12], label=round(ivout$ivtable[1:x_upper,12],2), pos=3,cex=1)
    abline(h=0)
    mtext(sub,3)
  } else {
    return("Options are dist, badrate, or WoE")
  }
}
# End Plotting ################################################################
